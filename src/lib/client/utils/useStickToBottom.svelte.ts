import { extract, watch, type Getter, type MaybeGetter } from "runed";
import { defaultWindow, type ConfigurableWindow } from "./runed";
 ...
 */
const isScrollable = function (el: HTMLElement) {
...
export function useStickToBottom(
  ...
  const handleWheel = ({ target, deltaY }: WheelEvent) => {
    const element = getScrollableParent(target as HTMLElement) ?? target;
